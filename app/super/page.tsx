...
export default function SuperCommandCenter() {
  ...
  const pickTab = useCallback((t: Tab) => {
    setTab(t);
    localStorage.setItem("cc_tab", t);
  }, []);
  const pickTabByKey = useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>, current: Tab) => {
      const index = TABS.findIndex((item) => item.v === current);
      const next =
      ...
      pickTab(next.v);
      document.getElementById(`super-tab-${next.v}`)?.focus();
    },
    [pickTab],
  );
  const pickRefresh = useCallback((r: string) => {
    setRefresh(r);
      ...
      <div className="max-w-md lg:max-w-6xl mx-auto px-4 pt-4">
        ...
        >
          <div>
            <div
              className="eye"
              style={{ color: GOLD, letterSpacing: ".12em" }}
            >
              TROPHĒ OPERATIONS
            </div>
            <div
  ...
  return (
    <div>
      ...
      >
        <Kpi
          label="Spend today"
          value={fmtUsd(cost("today")?.cost)}
          accent
          sub={`${fmtNum(cost("today")?.runs)} runs`}
        />
        <Kpi
          label="Spend 30d"
          value={fmtUsd(cost("30d")?.cost)}
          sub={`${fmtNum(cost("30d")?.runs)} runs`}
        />
        <Kpi
          label="Error rate 24h"
          value={`${errRate.toFixed(1)}%`}
          warn={errRate > 5}
          sub={`${data.aiErrors.errors_24h ?? 0}/${data.aiErrors.runs_24h ?? 0} runs`}
        />
        <Kpi
          label="AI latency p95 24h"
          value={fmtMs(data.aiErrors.p95_latency_24h)}
        />
        <Kpi
          label="Active clients 7d"
          value={fmtNum(data.activity.active_clients_7d)}
          sub={`${fmtNum(data.activity.logs_7d)} logs`}
        />
      ...
      <div className="lg:grid lg:grid-cols-2 lg:gap-4">
        <div>
          ...
          <Panel title="PEOPLE">
            ...
            <div
              className="ds-sub"
              style={{ fontSize: 12, marginTop: 10, fontFamily: MONO }}
            >
              Latest:{" "}
              {data.recentSignups
                .slice(0, 4)
        ...
        <div>
          {/* AI spend summary */}
          <Panel
            title="AI SPEND BY TASK · 7D"
            meta={
              <span
                className="ds-sub"
                style={{ fontSize: 12, fontFamily: MONO }}
              >
                full analytics in COSTS
              </span>
            }
          >
            {data.aiByTask.length === 0 ? (
              <Empty label="no runs" />
            ) : (
              data.aiByTask
                .slice(0, 8)
              ...
              data.recentFailures.map((f, i) => (
                ...
                >
                  <div className="row-b">
                    ...
                    >
                      {f.task} <StatusChip status={f.status} />
                    </span>
                    <span
                      className="ds-sub"
                      style={{ fontSize: 12, fontFamily: MONO }}
...
function DataSection({ data }: { data: Overview | null }) {
  const totalFoods = useMemo(
    () => data?.foods.reduce((s, f) => s + f.n, 0) ?? 0,
    [data],
  );
  const totalEmbedded = useMemo(
    () => data?.foods.reduce((s, f) => s + f.embedded, 0) ?? 0,
    [data],
  );
    ...
    <div>
      ...
      >
        ...
        <Kpi
          ...
          value={`${totalFoods ? Math.round((totalEmbedded / totalFoods) * 100) : 0}%`}
          sub={`${fmtNum(totalEmbedded)} embedded`}
        />
