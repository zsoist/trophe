...
export type LiveEvent =
  ...
  | { type: 'session.input_transcript.delta'; delta: string; startMs: number; endMs: number; eventId: string | null }
  | { type: 'session.output_transcript.delta'; delta: string; startMs: number; endMs: number; eventId: string | null }
...
/** Bounded optional provider event id: only a non-empty, short string is accepted.
 * This is a real documented field and is used as the caption dedup key. */
const optionalEventId = (value: unknown): string | null => {
...
export function parseLiveEvent(raw: unknown): LiveEvent | null {
  ...
  switch (value.type) {
    ...
    // The provider emits deltas only: event_id/delta/start_ms/end_ms. There is no
    // `.final` transcript event and no utterance id, so none is parsed or invented.
    ...
    case 'session.output_transcript.delta': {
      ...
      return { type: value.type, delta, startMs, endMs, eventId: optionalEventId(value.event_id) };
    }
