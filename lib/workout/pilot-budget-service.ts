...
import {
  type PilotBudgetResult, type PilotBudgetStore,
} from '@/agents/coach-assistant/pilot-budget';

import { multimodalCommandSchema, multimodalRecordSchema, decideMultimodalBudgetCommand, recordModality, type MultimodalRecord } from '@/agents/coach-assistant/multimodal-budget';
import type { AudioBudgetPort } from '@/agents/coach-assistant/audio-budget';
 ...
 */
function createMultimodalPilotBudgetStore(database: typeof db, actorId: string) {
  return {
    async execute(raw: unknown, signal: AbortSignal) {
      const parsed = multimodalCommandSchema.safeParse(raw);
      ...
      try {
        return await database.transaction(async transaction => {
          ...
          if (!config?.allowed || ('organizationId' in binding && binding.organizationId !== config.organization_id)) return fail('budget_blocked');
          ...
          let total = BigInt(0), turnAttemptCount = 0, turnChargedNanoUsd = 0, hasPendingAttempt = false, accountingBlocked = config.accounting_blocked;
          let existing: MultimodalRecord | undefined;
          for (const row of rows.rows) {
            const record = multimodalRecordSchema.safeParse(row.record);
            if (!record.success || record.data.binding.agentRunId !== row.id || record.data.binding.actorId !== row.user_id
              || record.data.binding.pilotId !== binding.pilotId || row.organization_id !== config.organization_id || row.model !== record.data.binding.model || ('organizationId' in record.data.binding && record.data.binding.organizationId !== config.organization_id)) return fail('uncertain');
            total += BigInt(record.data.chargedNanoUsd);
            if (record.data.binding.turnId === binding.turnId) {
              turnChargedNanoUsd += record.data.chargedNanoUsd;
              if (recordModality(record.data) === ('modality' in binding ? binding.modality : 'text')) turnAttemptCount++;
            }
            accountingBlocked ||= record.data.accountingAlert;
            hasPendingAttempt ||= record.data.state === 'dispatched' || record.data.state === 'unknown';
          ...
          const decision = decideMultimodalBudgetCommand({ pilotId: binding.pilotId, capNanoUsd: integer(config.cap_nano_usd), chargedNanoUsd: integer(config.charged_nano_usd), turnAttemptCount, turnChargedNanoUsd, accountingBlocked: accountingBlocked || hasPendingAttempt, existing }, command);
...
/** Both adapters lock and reconcile the SAME existing coach_pilot_budgets account. */
export function createPilotBudgetStore(database: typeof db, actorId: string): PilotBudgetStore {
  return createMultimodalPilotBudgetStore(database, actorId);
}
export function createAudioBudgetStore(database: typeof db, actorId: string): AudioBudgetPort {
  return createMultimodalPilotBudgetStore(database, actorId);
}
