...
The `coach_pilot` row is the financial carrier for one governed modality attempt. Its `provider` mirrors the provider implied by the immutable model binding: Anthropic for Claude Photo analysis and OpenAI for Luna or transcription. The linked task-specific row remains the authoritative provider execution record. Rows created before this rule may retain the legacy `openai` carrier label; their model, binding metadata, linked task row, usage, and settlement establish the actual modality and provider.
