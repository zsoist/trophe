...
withPool(resolveDbConfig(), async (pool) => {
  ...
  if (failureRate > maxFailureRate) failures.push(`failure rate ${(failureRate * 100).toFixed(1)}% exceeds ${(maxFailureRate * 100).toFixed(1)}%`);
  ...
  console.log(`AI budget gate passed: ${totalCalls} calls, $${totalCostUsd.toFixed(4)}, ${(failureRate * 100).toFixed(1)}% failures.`);
}).catch((error) => {
