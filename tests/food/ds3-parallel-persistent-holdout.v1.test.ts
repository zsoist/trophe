...
it.each([['search',2,0,false],['search',0,2,true],['model',2,0,true]] as const)('AG4 persistent counted %s searches%s models%s admits%s',async(kind,searches,models,allowed)=>{
 const base=binding(),organizationId=randomUUID();
 const search={...base,model:'parallel-search' as const,pricingVersion:'parallel-search-fast-2026-09-13' as const,reservedNanoUsd:1000000 as const};
 const input=kind==='search'?search:base;
 const records=Array.from({length:searches+models},(_,i)=>{const b={...(i<searches?search:base),attemptId:randomUUID(),agentRunId:randomUUID()};return {id:b.agentRunId,user_id:actor,organization_id:organizationId,model:b.model,record:{binding:b,admissionDay:'2026-09-13',state:'reserved',chargedNanoUsd:b.reservedNanoUsd,usage:null,accountingAlert:false}};});
 const charged=records.reduce((n,r)=>n+r.record.chargedNanoUsd,0);
 const execute=vi.fn().mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[{organization_id:organizationId,cap_nano_usd:'3000000000',operating_target_nano_usd:'500000000',budget_day:'2026-09-13',server_budget_day:'2026-09-13',charged_nano_usd:String(charged),attempt_count:records.length,accounting_blocked:false,allowed:true}]}).mockResolvedValueOnce({rows:[{id:actor}]}).mockResolvedValueOnce({rows:records}).mockResolvedValue({rows:[]});
 const transaction=vi.fn(async work=>work({execute}));
 const result=await createPilotBudgetStore({transaction} as unknown as typeof db,actor).execute({operation:'reserve',binding:input},new AbortController().signal);
 expect(result).toMatchObject({ok:allowed});if(!allowed)expect(result).toMatchObject({error:'budget_blocked'});
});
