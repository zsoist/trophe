...
describe('cache and provenance boundaries', () => {
  ...
  it('never serves a cached payload whose identity differs from the request', async () => {
    const ids = identity(), store = memoryStore(), transport = transportOf();
    // Simulates a contaminated/mismatched entry surviving under the requested
    // key: serving it would hand the model another product's evidence.
    const contaminated = {
      provider: 'parallel' as const, endpoint: 'https://api.parallel.ai/v1/search',
      product: 'B|C', brand: 'A', locale: 'en-US', market: 'United States',
      location: 'us', hl: 'en', objective: 'stale', query: 'stale', searchId: null,
      fetchedAt: new Date(0).toISOString(), results: [],
    };
    const cache = { get: () => contaminated, set: () => undefined };
    const result = await runNutritionSearch(
      { product: 'C', brand: 'A|B', locale: 'en-US' },
      { transport, store, cache, ...ids, signal: new AbortController().signal },
    );
    const evidence = okEvidence(result);
    expect(evidence.product).toBe('C');
    expect(evidence.brand).toBe('A|B');
    expect(transport.search).toHaveBeenCalledTimes(1);
  });

  it('normalizes case and surrounding whitespace to the same cache identity', async () => {
    const ids = identity(), store = memoryStore(), transport = transportOf();
    const cache = createNutritionSearchCache({ now: () => 0 });
    const deps = { transport, store, cache, ...ids, signal: new AbortController().signal };
    expect((await runNutritionSearch({ product: 'Big Mac', brand: null, locale: 'en-US' }, deps)).status).toBe('ok');
    const second = await runNutritionSearch({ product: '  big mac  ', brand: null, locale: 'en-US' }, deps);
    expect(second).toMatchObject({ status: 'ok', cached: true, creditConsumed: false });
    expect(transport.search).toHaveBeenCalledTimes(1);
  });
});
