      ...
      {mode === 'edit' ? (
        <>
          <div className="plan-exercise-card__prescription">
            <label><span>{t('workout.target_sets')}</span><input className="text-base" type="number" inputMode="numeric" min={1} max={20} disabled={locked} aria-label={t('workout.target_sets_named', { name })} value={draftExercise.targetSets} onChange={(event) => onUpdate?.({ targetSets: Math.max(1, Math.min(20, Number(event.target.value) || 1)) })} /></label>
            <label><span>{t('workout.target_reps')}</span><input className="text-base" disabled={locked} aria-label={t('workout.target_reps_named', { name })} value={draftExercise.targetReps} onChange={(event) => onUpdate?.({ targetReps: event.target.value })} /></label>
            <label><span>{t('workout.rest_seconds')}</span><input className="text-base" type="number" inputMode="numeric" min={0} max={600} step={15} disabled={locked} aria-label={t('workout.rest_seconds_named', { name })} value={restSeconds} onChange={(event) => onUpdate?.({ restSeconds: Math.max(0, Math.min(600, Math.round(Number(event.target.value) || 0))) })} /></label>
            <label><span>{t('workout.target_rpe')}</span><input className="text-base" type="number" inputMode="decimal" min={1} max={10} step={0.5} disabled={locked} placeholder="—" aria-label={t('workout.target_rpe_named', { name })} value={rpe ?? ''} onChange={(event) => onUpdate?.({ targetRpe: event.target.value === '' ? null : Math.max(1, Math.min(10, Number(event.target.value))) })} /></label>
          </div>
