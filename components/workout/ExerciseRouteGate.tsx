...
export function ExerciseRouteGate({ actionLabel, onAction }: { actionLabel: string; onAction: () => void }) {
  const { t } = useI18n();
  return (
    <main className="mx-auto flex min-h-[calc(100dvh-8rem)] w-full max-w-3xl items-center px-4 py-10">
      <section role="status" className="w-full rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-6 text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[var(--content-secondary)]">{t('workout.exercise_requires_strength_draft')}</p>
