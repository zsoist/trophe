...
const emptyData: WorkoutAnalyticsData = {
  ...
  measurements: [],
  programs: [],
  issues: { schedule: false, measurements: false },
};
...
export default function WorkoutAnalyticsSurface() {
  ...
  const indexed = useMemo(() => {
    ...
    const entries = new Map<string, { date: string; exercise: WorkoutAnalyticsData['sets'][number]['exercise']; sets: Array<{ completed: boolean; isWarmup: boolean }> }>();
    ...
    for (const set of data.sets) {
      ...
      const entry = entries.get(entryKey) ?? { date: set.session.session_date, exercise: set.exercise, sets: [] };
  ...
  const latestCompletedDate = data.sessions[0]?.session_date;
  ...
  return <div className="min-h-screen bg-[var(--canvas)] pb-[calc(7rem+env(safe-area-inset-bottom))]">
    <main className="mx-auto max-w-3xl px-4 py-6 sm:px-6">
      <header className="mb-7 flex items-end justify-between gap-4"><div><h1 className="text-2xl font-semibold tracking-[-0.02em] text-[var(--content-primary)]">{t('workout.analytics_title')}</h1><p className="mt-1 text-sm text-[var(--content-muted)]">{t('workout.analytics_intro')}</p></div><Link href="/dashboard/workout/history" className="inline-flex min-h-11 items-center px-3 text-sm font-medium text-[var(--action-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]">{t('workout.history')}</Link></header>
      {loading ? <p role="status" className="py-16 text-center text-sm text-[var(--content-muted)]">{t('workout.analytics_loading')}</p> : error ? <section role="alert" className="rounded-xl bg-[var(--surface-subtle)] p-5"><AlertCircle size={20} className="mb-2 text-[var(--status-danger-fg)]" /><p className="text-sm text-[var(--content-secondary)]">{t('workout.analytics_load_failed')}</p><button type="button" onClick={() => void load()} className="mt-4 min-h-11 rounded-lg px-3 text-sm font-medium text-[var(--action-primary)] outline outline-1 outline-[var(--border-default)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]">{t('workout.retry')}</button></section> : <div className="space-y-7">
        {hasPartialIssue ? <section role="alert" className="rounded-xl bg-[var(--surface-subtle)] p-4 text-sm text-[var(--status-danger-fg)]">{data.issues.schedule ? <p>{t('workout.analytics_schedule_failed')}</p> : null}{data.issues.measurements ? <p>{t('workout.analytics_body_weight_failed')}</p> : null}<button type="button" onClick={() => void load()} className="mt-3 min-h-11 px-3 font-medium text-[var(--action-primary)]">{t('workout.retry')}</button></section> : null}
