...
export function WorkoutSummaryMetrics({ sessions, unit = 'kg' }: { sessions: WorkoutSummarySession[]; unit?: WeightUnit }) {
  ...
  const metrics = [{ label: 'Duration', value: `${duration} min` }, { label: 'Working sets', value: `${completed.length} working set${completed.length === 1 ? '' : 's'}` }, { label: 'Lifted volume', value: `${kgToDisplay(volume, unit)} ${unit}` }, { label: 'Personal records', value: `${prs} PR${prs === 1 ? '' : 's'}` }];
