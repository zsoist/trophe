    ...
    <motion.div initial={reducedMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={reducedMotion ? undefined : { opacity: 0 }} className="fixed inset-0 z-[var(--z-modal,60)] flex items-end justify-center" style={{ background: 'var(--surface-overlay)' }} onClick={() => { if (!addingWarmups) onCloseRef.current(); }}>
      <motion.div ref={dialogRef} role="dialog" aria-modal="true" aria-label={t('workout.plate_title')} tabIndex={-1} onKeyDown={(event) => trapFocus(event, dialogRef.current)} initial={reducedMotion ? false : { y: 32, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={reducedMotion ? undefined : { y: 24, opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.22, ease: [0.16, 1, 0.3, 1] }} className="workout-workspace glass-elevated safe-bottom w-full max-w-md rounded-t-2xl px-5 pt-4 pb-[calc(5rem+env(safe-area-inset-bottom))] outline-none" onClick={(event) => event.stopPropagation()}>
        ...
        <section className="mt-5" aria-labelledby="warmup-title">
          ...
          {warmupRamp.length ? <div className="mt-2 grid grid-cols-3 gap-2">{warmupRamp.map((set) => <div key={set.percentage} className="rounded-xl bg-[var(--surface-subtle)] p-2 text-center"><div className="text-xs font-semibold text-[var(--content-secondary)]">{set.achievedPercentage}%</div><div className="mt-0.5 text-sm font-bold tabular-nums text-[var(--content-primary)]">{set.weight} {unit}</div><div className="text-xs text-[var(--content-muted)]">× {set.reps}</div></div>)}</div> : <p className="mt-2 text-xs text-[var(--content-muted)]">{t('workout.warmup_no_ramp')}</p>}
