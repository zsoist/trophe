...
export default function MonthlyReport({ userId, targets }: MonthlyReportProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    // Aggregate current month by date
    ...
    for (const entry of currentRes.data ?? []) {
      ...
      if (!dateMap[d]) dateMap[d] = { calories: 0, protein: 0, carbs: 0, fat: 0 };
      dateMap[d].calories += entry.calories ?? 0;
      dateMap[d].protein += entry.protein_g ?? 0;
      dateMap[d].carbs += entry.carbs_g ?? 0;
      dateMap[d].fat += entry.fat_g ?? 0;
    }

    const loggedDates = Object.entries(dateMap).filter(([, v]) => v.calories > 0);
    const daysLogged = loggedDates.length;
    const consistency = daysSoFar > 0 ? Math.round((daysLogged / daysSoFar) * 100) : 0;

    const avgCalories = daysLogged > 0 ? Math.round(loggedDates.reduce((s, [, v]) => s + v.calories, 0) / daysLogged) : 0;
    const avgProtein = daysLogged > 0 ? Math.round(loggedDates.reduce((s, [, v]) => s + v.protein, 0) / daysLogged) : 0;
    const avgCarbs = daysLogged > 0 ? Math.round(loggedDates.reduce((s, [, v]) => s + v.carbs, 0) / daysLogged) : 0;
    const avgFat = daysLogged > 0 ? Math.round(loggedDates.reduce((s, [, v]) => s + v.fat, 0) / daysLogged) : 0;
    ...
    // Adherence
    const adherenceScores = loggedDates.map(([, v]) => dayScore(v.calories, v.protein, targets.calories, targets.protein_g));
    const avgAdherence = adherenceScores.length > 0
      ? Math.round(adherenceScores.reduce((s, v) => s + v, 0) / adherenceScores.length)
      : 0;
    ...
    const prevLogged = Object.values(prevDateMap).filter((v) => v > 0);
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <MetricCard
          ...
          trend={<TrendIcon current={stats.consistency} prev={stats.prevConsistency} />}
        />
        <MetricCard
          label="Avg Calories"
          value={`${stats.avgCalories}`}
          sub="kcal"
          trend={<TrendIcon current={stats.avgCalories} prev={stats.prevAvgCalories} />}
        />
        <MetricCard
          label="Avg Protein"
