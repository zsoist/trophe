...
interface CostsData {
  ...
  breakdown: Array<{ key: string; runs: number; cost: number; tokens_in: number; tokens_out: number; avg_latency: number | null; failed: number }>;
  daily: Array<{ day: string; cost: number; runs: number }>;
  topRuns: Array<{ id: string; task: string; model: string; cost: number; tokens_in: number; tokens_out: number; latency_ms: number | null; status: string; created_at: string }>;
  facets: { providers: string[]; models: string[]; tasks: string[] };
}

const WINDOW_OPTS = [
  { v: '24h', label: '24H' }, { v: '7d', label: '7D' },
  { v: '30d', label: '30D' }, { v: '90d', label: '90D' }, { v: 'all', label: 'ALL' },
] as const;

const GROUP_OPTS = [
  { v: 'model', label: 'MODEL' }, { v: 'provider', label: 'PROVIDER' },
  { v: 'task', label: 'TASK' }, { v: 'user', label: 'USER' }, { v: 'status', label: 'STATUS' },
] as const;

export default function CostsPanel() {
  const [data, setData] = useState<CostsData | null>(null);
  const [win, setWin] = useState<string>(() =>
    typeof window !== 'undefined' ? localStorage.getItem('cc_costs_window') ?? '7d' : '7d');
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { localStorage.setItem('cc_costs_window', win); }, [win]);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 12 }}>
        ...
        <Pills options={[...GROUP_OPTS]} value={groupBy} onChange={setGroupBy} small />
        <Select value={provider} onChange={setProvider} options={data?.facets.providers ?? []} allLabel="all providers" />
        <Select value={model} onChange={setModel} options={data?.facets.models ?? []} allLabel="all models" />
        <Select value={task} onChange={setTask} options={data?.facets.tasks ?? []} allLabel="all tasks" />
          ...
          <div>
            {(data?.breakdown ?? []).map((b) => (
              <HBar
                ...
                color={b.failed > 0 && b.failed / b.runs > 0.1 ? '#EF6A5A' : undefined}
              />
            ))}
          </div>
        )}
      ...
      <Panel title="MOST EXPENSIVE RUNS">
        {(data?.topRuns ?? []).length === 0 ? <Empty label={data ? 'no runs' : 'loading…'} /> : (
          <TableWrap maxHeight={320}>
            <thead>
              <tr>
                <Th>Task</Th><Th>Model</Th><Th right>Cost</Th><Th right>Tok in/out</Th>
                <Th right>Latency</Th><Th>Status</Th><Th right>When</Th>
              </tr>
            </thead>
            <tbody>
              {(data?.topRuns ?? []).map((r) => (
                <tr key={r.id}>
                  ...
                  <Td right mono dim>{new Date(r.created_at).toLocaleString()}</Td>
                </tr>
              ))}
            </tbody>
          </TableWrap>
