...
export function PhotoFoodPanel({controller,state,transport,onReceipt,onOpenFoodLog}:{controller:PhotoFoodController;state:PhotoFoodState;transport:PhotoFoodTransport;onReceipt?:(entryId:string)=>void;onOpenFoodLog?:()=>void}){
 ...
 return <section className={styles.foodReview} aria-label={t('global_coach.photo_food_title')}>
  ...
  {state.snapshot&&<><p>{t(state.snapshot.source==='offline_fixture'?'global_coach.photo_food_offline':'global_coach.photo_food_observation')}</p><p>{t('global_coach.photo_food_estimate')}</p>
   {state.snapshot.items.map(item=><button key={item.index} type="button" aria-pressed={item.index===state.itemIndex} onClick={()=>controller.choose(item.index)}>{item.foodName} · {item.identityStatus==='identified'?`${item.estimatedGrams} g · ${item.estimatedCalories} kcal`:t(item.identityStatus==='uncertain'?'global_coach.photo_food_hypothesis':'global_coach.photo_food_unassessed')}</button>)}
  </>}
  ...
  {state.pending&&<p role="status">{t('global_coach.pending')}</p>}{state.error&&!state.uncertain&&!(selected&&!identityReady&&state.error==='identity_clarification_required')&&<p role="status">{t(state.error==='identity_clarification_required'?'global_coach.photo_food_identity_required':'global_coach.photo_food_error')}</p>}
 </section>;
}
