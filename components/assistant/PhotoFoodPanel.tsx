...
export function PhotoFoodPanel({controller,state,transport,onReceipt,onOpenFoodLog}:{controller:PhotoFoodController;state:PhotoFoodState;transport:PhotoFoodTransport;onReceipt?:(entryId:string)=>void;onOpenFoodLog?:()=>void}){
 ...
 return <section className={styles.foodReview} aria-label={t('global_coach.photo_food_title')}>
  ...
  {state.snapshot&&<><p>{t(state.snapshot.source==='offline_fixture'?'global_coach.photo_food_offline':'global_coach.photo_food_observation')}</p><p>{t('global_coach.photo_food_estimate')}</p>
   {state.snapshot.items.map(item=><button key={item.index} type="button" aria-pressed={item.index===state.itemIndex} onClick={()=>controller.choose(item.index)}>{item.identityStatus==='identified'?item.foodName:t('global_coach.photo_food_unidentified', {number:item.index+1})} · {item.identityStatus==='identified'?`${item.estimatedGrams} g · ${item.estimatedCalories} kcal`:t(item.identityStatus==='uncertain'?'global_coach.photo_food_hypothesis':'global_coach.photo_food_unassessed')}</button>)}
  </>}
  {selected&&!state.receipt&&<p>{selected.accuracyNote}</p>}
