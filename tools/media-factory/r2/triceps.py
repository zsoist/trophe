...
def run(config,out):
    ...
    def pose(frame):
        ...
        prop.matrix_world=Matrix.Translation((0,e.y+radius*math.sin(angle)+.065,e.z+radius*math.cos(angle)+.013))
        bpy.context.view_layer.update()
        for side in ['L','R']:
    ...
    for side,sign in [('L',1),('R',-1)]:
        for finger in ['f_index','f_middle','f_ring','f_pinky']:
            for part in range(1,4):
                ...
                edge=math.sqrt(max(.001,.085**2-h.x*h.x))
                if part==1:target=Vector((h.x,edge+.003,-.012))
                elif part==2:target=Vector((h.x,edge+.008,.003))
                else:target=Vector((h.x,edge+.008,.034))
