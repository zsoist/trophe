    ...
    if config.get('full_body_framing_gate'):
        ...
        for frame in sorted(set(list(range(1,scene.frame_end+2,30))+config.get('stills',[]))):
            ...
            for obj in scene.objects:
                ...
                if obj.name not in ['Trophe_R2_Athlete','Trophe_R2_Trainers','SportsTank','SportsShorts'] and not obj.name.startswith(('Copa weight','Copa bench','Incline weight','Incline bench')+tuple(config.get('framing_object_prefixes',[]))):continue
