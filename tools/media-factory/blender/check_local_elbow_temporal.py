...
data = {}
transforms = {}
report = {'references': {'original_rest': 'Immutable original skeleton bone.matrix_local; used only to select fixed anatomical vertex IDs, not the temporal denominator.', 'animated_frame1': 'Evaluated authoring pose at t=0, frame1. Native modifier bound to this surface; authoring skeleton remains unchanged.', 'initial_surface': 'Original base frame1 shape keys + PreserveVolume armature; masks/subdivision disabled only for stable vertex correspondence. Common reference for BOTH stages.'}, 'sources': {}, 'regions': {}, 'stages': {}}
for stage, folder in [('base', 'refine-v3-02'), ('local_native', 'elbow-local-native-01')]:
    ...
    for m in h.modifiers:
        if m.type not in ['ARMATURE', 'CORRECTIVE_SMOOTH']:
...
report['authoring_bones_grip_prop_fullmatrix_max_delta'] = float(np.abs(transforms['local_native'] - transforms['base']).max())
assert report['authoring_bones_grip_prop_fullmatrix_max_delta'] < 1e-06
report['visible_skin_frame1_max_delta_m'] = float(np.linalg.norm(data['local_native'][0, list(visible)] - data['base'][0, list(visible)], axis=1).max())
...
np.savez_compressed(out / 'evaluated-skin-181.npz', base=data['base'], local_native=data['local_native'])
...
modified = {i for r in recipe['regions'].values() for ring in [r['core_ids']] + r['feather_rings'] for i in ring}
outside = sorted(set(range(data['base'].shape[1])) - modified)
delta = np.linalg.norm(data['local_native'][:, outside] - data['base'][:, outside], axis=2)
preserve = {'method': 'All181 poses, same source IDs; vertices outside mapped core+two feather rings', 'vertices_outside': len(outside), 'max_delta_m': float(delta.max()), 'frame1_all_vertices_max_delta_m': float(np.linalg.norm(data['local_native'][0] - data['base'][0], axis=1).max())}
assert preserve['max_delta_m'] < 1e-06, preserve
(out / 'outside-preservation.json').write_text(json.dumps(preserve, indent=2))
print('OUTSIDE', preserve)
