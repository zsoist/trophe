...
def tank(p):
 x,y,z=p
 if z<.995 or z>1.475:return False
 if abs(x)>.225:return False
 if z>1.405 and abs(x)<.105:return False
...
for name,predicate,mat in [('SportsTank',tank,shirtmat),('SportsShorts',shorts,shortmat)]:
 ...
 for i in ids:
  ...
  verts.append(p)
 mesh=bpy.data.meshes.new(name);mesh.from_pydata(verts,[],[[mapping[i] for i in f] for f in faces]);mesh.update();o=bpy.data.objects.new(name,mesh);bpy.context.collection.objects.link(o);o.data.materials.append(mat)
...
skin=bpy.data.materials.new('Skin - warm medium controlled');skin.use_nodes=True;p=skin.node_tree.nodes.get('Principled BSDF');p.inputs['Base Color'].default_value=(.30,.135,.074,1);p.inputs['Roughness'].default_value=.48;p.inputs['Subsurface Weight'].default_value=.06
