...
export function PrivateFood() {
  ...
  return <main className="mx-auto max-w-2xl space-y-6 px-4 pb-8 pt-20">
    ...
    <QuickFoodInput userId={REVIEW_USER} mealType="lunch" date={localToday()} showCalories onLogged={() => {}} onSearchMode={() => {}} />
    {foods.length > 0 && <section aria-label={t('global_coach.food')} className="divide-y divide-[var(--border-default)]">{foods.map(food => <p key={food.id} className="py-3">{food.food_name}<span className="block text-sm text-[var(--text-secondary)]">{food.calories} kcal · {food.protein_g} g {t('food.protein')}</span></p>)}</section>}
  </main>;
}
