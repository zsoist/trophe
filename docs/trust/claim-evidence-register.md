...
| RLS-ISOLATION | *(not claimed absolutely)* coach/client cross-tenant denial on sensitive resources is technically verified | `verified-technical` | WP2: `tests/db/rls.test.ts` assigned-vs-unassigned matrix for profiles, orgs, food/water/measurements, coach notes, messages, meal plans, supplements, workouts, form analyses; `tests/enterprise/invariants.test.ts` Drizzle RLS mirror fail-closed guard; `scripts/db/verify.ts` no `TO public` policies + anon grant allowlist | 2026-09-15 |
