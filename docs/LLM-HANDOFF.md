...
Production food parsing loads `agents/prompts/food-parse.v9.md` by default from `agents/food-parse/index.v4.ts`. The `index.v4.ts` filename describes pipeline architecture, not prompt version; do not infer that production uses the v4 prompt. v9 retains the nutrition-label safeguards from v8 and explicitly separates cooked steak (25–30 g protein/100 g) from 80/20 ground beef while preserving explicit gram portions. The routing decision and Phase 2 evidence are in `artifacts/phase2/phase2-decision-report.md`.
