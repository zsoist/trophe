...
# Remediation Addendum - 2026-06-08

This audit was the input to the production-hardening work completed on branch
`codex/trophe-100-execution`. Findings describing migrations as undeployed or
organization AI budgets as absent are historical and have now been remediated.

Verified production state:

- Migrations through `0017_organization_ai_budgets.sql` are applied to Supabase.
- Production DB verification passes, including RLS posture and governed AI tables.
- Invalid Sonnet routing was replaced with `claude-sonnet-4-6`.
- Gemini 2.5 Flash and Voyage 4 cost pricing was corrected.
- Every routed model must have non-zero input pricing in tests.
- Organization AI daily/monthly budgets and kill switches are enforced before provider calls.
- Admin cost reporting is tenant-scoped; global organization visibility is super-admin only.
- Cost reporting includes task, model, user, cache, reliability, and latency percentile data.
- Permission-aware RAG, audited privacy requests, and governed AI generation persistence are live.
- Production deployment `dpl_GxaLN37b22H9uhrF1U2UmX7csryJ` is aliased to `https://trophe.app`.
- Read-only production canary and production DB verification pass.
- Full local release gate passes: typecheck, lint, 251 tests, and production build.
- Browser E2E passes 8 public/anonymous flows; 8 authenticated role flows remain skipped because
  dedicated E2E credentials are not configured in this execution environment.
- Live model evals remain an explicit release-evidence gap because eval authentication/provider
  credentials are not configured in this execution environment.
- Apple Watch/wearable provider integration remains intentionally on hold.
- Dependency audit has no high/critical findings; two moderate PostCSS findings remain nested in
  Next.js and cannot be fixed without an unsafe forced downgrade.

The product is deployed and materially hardened, but should not be represented as literally
"100/100" until authenticated role E2E and live provider eval suites run successfully.
