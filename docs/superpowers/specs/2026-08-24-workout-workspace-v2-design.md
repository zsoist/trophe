...
**Verified code release candidate:** `0420b39051a77d60a00525e47899fad5d38eb22d`
**Local verification date:** 2026-08-25
**Included native-4K asset commit:** `2382140`

- Static gates: `git diff --check`, TypeScript, and ESLint all exited 0; ESLint reported 45 existing repository warnings and 0 errors, with no warning introduced by this round.
- Round-six focused workspace, confirmation, localization, and authenticated Postgres gates passed 8 files and 65 tests. The terminal parent identity/catalog subset passed 20/20, including legitimate template archival.
- Round-seven terminal-delete and manual-warm-up focused gates passed 4 files and 37 tests. The real consumer proof covers warm-up insertion, fresh shifted work-set identity, exact undo identity, finish eligibility, and remount recovery.
- Full Vitest under Node 20.20.0 passed: 267 files and 2,040 tests passed; 1 file and 46 tests were intentionally skipped.
- `db:bootstrap`, `db:verify`, and `db:explain` passed. Migrations 0080 and 0081 replay idempotently and their catalog proofs retain restricted, pinned-search-path authority. Local Postgres proof covers a commit-but-response-lost retrospective save, exact canonical retry, one-way terminal authority, rejection of reopening `duration_minutes`, rejection of every terminal set/parent identity mutation, legitimate live-session writes, FK-safe template archival, rejection of direct completed strength/cardio deletion, terminal-safe discard, active empty cleanup, and account-erasure cascade behavior.
- Production build: Next.js 16.2.12 compiled successfully and generated 70 static pages using the local loopback Supabase environment. The initial shell-only attempt without `NEXT_PUBLIC_SUPABASE_URL` stopped during prerender before the documented local environment was supplied.
- Performance, theme, paid-AI, and workout-asset guards passed. The deterministic asset check covered 20 native SVG sources, true 4K no-upscale masters, 232,632 runtime bytes, and stable hashes.
- Authenticated Playwright: the exact Workout Workspace V2 suite passed 22/22 across mobile and desktop projects and removed its disposable users. It includes a retrospective commit-response-lost/remount/retry journey, structured cardio History Repeat, and manual warm-up insertion with shifted work-row identity retained across reload. Completed cardio never exposes an empty-session discard. The journeys made no paid-provider requests.
...
- Visual evidence: original-resolution captures cover Home, Preview, Build, Browser, Detail, Review, Live, Paused, Pain, Plate, Finish, Completed, responsive 320/375/390/430 layouts, and the single-header History/Stats/Form Check support routes in light and dark. The final 390×844 English Form Check captures are retained under `test-results/workout-workspace-v2-Worko-f5d03-nd-does-not-scroll-sideways-mobile-chromium/` and `test-results/workout-workspace-v2-Worko-bbf5f-nd-does-not-scroll-sideways-mobile-chromium/`.
...
The final recovery contract retains Completed until explicit acknowledgement, routes active and terminal stages to their dominant Continue/Summary action, preserves route-aware Back separately from Workout Home, and keeps draft edits intact. Choosing a different template, program, or routine while a draft exists now requires a named replacement confirmation; Cancel preserves the current draft. A prepared start request is immutable after an ambiguous failure: editing stays locked and Retry replays the exact envelope. Shared draft validation prevents blank prescriptions from being persisted or submitted. Saved plans use awaited owner-scoped persistence; failed set payloads survive retry/refresh; every terminal set mutation is database-protected; cardio recovery and History use structured available facts; and Build/Review show movement, muscle, and equipment evidence. The shared confirmation sheet now mounts its portal inside its presence boundary so the production modal retains backdrop, focus, Escape, and focus-trap behavior. The scoped runner forwards explicit Playwright specs and options verbatim while preserving the existing default matrix when no arguments are supplied. The final visual pass also keeps the install offer in document flow, restores the persisted kg/lb control exclusively on Workout Home, prevents bottom-navigation collisions at 320–390 px, and removes inherited page-height geometry from Pain and Plate dialogs.

Round four closes the remaining recovery edges. Exercise browser/detail deep links cannot mutate or navigate as though Add succeeded while a prepared live-start request is unresolved. Recoverable legacy fractional set targets use a deterministic nearest-integer normalization (`2.5` becomes `3`) without discarding the surrounding workspace, while newly entered targets remain strict positive integers. Confirmation actions have a synchronous in-flight guard, so a same-tick double tap performs exactly one mutation and a failed async action can be retried. Retrospective logging now persists one typed immutable request envelope before transport, freezes editing/navigation while the result is ambiguous, replays the exact envelope after remount, reconciles the canonical completed session after a lost response, and never reuses that key for live start or a different payload.

Round five closes the final data and navigation blockers. A reconciled retrospective completion renders from its persisted immutable envelope and never requests a nonexistent live structure; it remains accurate after remount. Migration 0079 adds database-owned one-way `completed_at` terminal authority, so an authenticated owner cannot clear duration to reopen a session or mutate terminal evidence. Legacy fractional drafts remain recoverable while only impossible unsent legacy start envelopes are invalidated. History Repeat reconstructs a structured cardio draft and enters Review, while pending retrospective requests guard every query-loader path. Explicit Repeat is also an atomic acknowledgement-by-intent of an already confirmed Completed summary. History, Stats, and Form Check now use one canonical localized workspace header and route-aware Back/Home controls instead of duplicating nested headers. Original-resolution inspection additionally caught and closed a 320px History metadata/summary overlap; its release assertion now proves the two grid rows do not intersect or exceed the card.

Round six retires a live-start retry envelope only after the canonical session identifier is accepted, so ambiguous failures remain exactly replayable while completed History Repeat can safely create a new editable plan. Strength and cardio retrospective confirmations now use the shared accessible confirmation sheet with safe initial focus, Tab/Shift+Tab containment, Escape cancellation, in-flight double-submit protection, retry after failure, and trigger focus restoration. Migration 0080 freezes completed-session identity, creation order, and template provenance against authenticated owner updates while preserving template deletion through a restricted security-definer archival trigger. Form Check is now localized end to end in English, Spanish, Greek, French, German, Italian, Portuguese, and Dutch, including exercise choices, camera/runtime states, privacy copy, analysis labels, results, and errors; the English 390×844 light/dark evidence contains no Spanish fallback.

Round seven closes two final integrity holes. Migration 0081 adds a `BEFORE DELETE` terminal-session guard, so an authenticated owner cannot remove completed strength or zero-set cardio history; `discard_empty_workout_session` now accepts only an owned, nonterminal, no-set session that has not entered finishing, while trusted maintenance and account-erasure cascades remain intact. Manual warm-up completion now tracks the originating logical set number: when the persisted warm-up is inserted before planned work, the shifted work row resets to a fresh editable identity instead of reusing the warm-up database id. Undo targets the exact saved row, and recovery remounts the warm-up and work sets without poisoning finish eligibility.
