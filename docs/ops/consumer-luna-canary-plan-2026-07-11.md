...
- Every consumer food-parse request is stamped server-side with `canarySegment=consumer-luna-week-1`. Only the optional `evalSuite` request header requires a user listed in `AI_RATE_LIMIT_BYPASS_USER_IDS`, so ordinary users cannot impersonate a probe suite. Langfuse receives a stable SHA-256 pseudonymous user key, never the raw UUID.
...
2. Malformed or empty food-parse outputs exceed 1% of final request outcomes in the segmented production lane. Query only `agent_runs` rows where `metadata.canarySegment = 'consumer-luna-week-1'` and `metadata.apiOutcome` is present; this includes ordinary consumer traffic, excludes intermediate primary/fallback attempts, and counts the final API result once.
