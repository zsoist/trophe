...
export type CoachVoiceResult={version:'coach-assistant.voice.v1';ok:false;status:'not_connected'|'error';error:'budget_blocked'|'accounting_uncertain'|'forbidden'|'invalid_audio'|'invalid_input'|'invalid_output'|'cancelled'|'deadline'|'provider_unavailable'}|
  {version:'coach-assistant.voice.v1';ok:true;status:'review_required';scope:CoachVoiceScope;turnId:string;transcript:{text:string;locale:string;languages:string[];source:'synthetic_fixture';trust:'untrusted_transcript'};review:{token:string;expiresAt:string;editable:true;audioRetention:'discarded_after_transcription'};durationMs:number};
