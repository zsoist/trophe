...
function fixture() {
  ...
  return {execute,missing:()=>{missing=true;},revoke:()=>{authorized=false;},loseCommit:()=>{lostCommit=true;},expireProposal:(proposalId:string)=>{if(proposals[proposalId])proposals[proposalId].expired=true;},state:()=>({row,revision,receipts,proposals}),failReceipt:()=>{receiptFails=true;},manualEdit:()=>{row.dietPattern='vegan';revision++;}};
}
