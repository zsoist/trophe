...
export const openConversationSchema=z.object({
  ...
  limitations:z.array(z.string().max(160)).max(8),escalation:z.boolean(),
}).strict();
 ...
 */
export async function generateOpenConversation(input:CoachConversationRequest,response:CoachConversationResponse,provider:OfflineConversationProvider,signal:AbortSignal):Promise<void> {
  ...
  if(new TextEncoder().encode(COACH_CONVERSATIONAL_SYSTEM_PROMPT+prompt+JSON.stringify(schema)).length>7500)throw new Error('context_limit');
  ...
  try { generated=await provider({policy:{...taskPolicies.coach_assistant,promptVersion:COACH_CONVERSATIONAL_PROMPT_VERSION},system:COACH_CONVERSATIONAL_SYSTEM_PROMPT,prompt,schema,validator:openConversationSchema,signal,maxTokens:2000,maxAttempts:1,store:false}); }
  ...
  response.output={answer:`Synthetic provider fixture evaluation. Interpretation for review: ${output.answer}${canonicalFacts.length?'\nRecorded facts:\n'+canonicalFacts.join('\n'):''}`,evidenceRefs:output.evidenceRefs,
