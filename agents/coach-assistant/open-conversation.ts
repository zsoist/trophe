 ...
 */
export type OfflineInterpretationReview=(input:{answer:string;evidenceRefs:string[];evidence:CoachEvidence[];signal:AbortSignal})=>Promise<{approved:boolean}>;
...
/** Conservative speech-act gate for the current offline evaluation.
 * Free declarative interpretations are not released as if citations proved them.
 * This is a bounded linguistic contract, not general semantic verification.
 */
function isNonDeclarativeProse(text:string):boolean {
  const sentences=text.trim().split(/(?<=[.!?])\s+/u);
  return sentences.every(sentence=>{
    const normalized=sentence.normalize('NFKD').replace(/\p{M}/gu,'').trim();
    if(/[;:\n]/.test(normalized))return false;
    const question=/^[¿]?(?:what|which|how|would you|could you|do you|que|cual|como|te gustaria|podrias)\b/i.test(normalized)&&normalized.endsWith('?');
    const suggestion=/^(?:consider (?:reviewing|checking|asking|discussing|choosing|planning)|you could (?:review|check|ask|discuss|choose|plan)|podrias (?:revisar|comprobar|preguntar|conversar|elegir|planificar)|puedes considerar)\b/i.test(normalized);
...
export async function generateOpenConversation(input:CoachConversationRequest,response:CoachConversationResponse,provider:OfflineConversationProvider,signal:AbortSignal,reviewInterpretation?:OfflineInterpretationReview):Promise<void> {
  ...
  if(output.followUp!==null&&!isNonDeclarativeProse(output.followUp))throw new Error('invalid_output');
  const declarative=!isNonDeclarativeProse(output.answer);
  if(declarative&&!reviewInterpretation)throw new Error('invalid_output');
  ...
  if(declarative) {
    const review=await reviewInterpretation!({answer:output.answer,evidenceRefs:[...output.evidenceRefs],evidence:structuredClone(facts),signal});
    signal.throwIfAborted();
  ...
  response.output={answer:`Synthetic provider fixture evaluation. ${declarative?'Offline oracle-reviewed interpretation':'Suggestion or question for review'}: ${output.answer}${canonicalFacts.length?'\nRecorded facts:\n'+canonicalFacts.join('\n'):''}`,evidenceRefs:output.evidenceRefs,
