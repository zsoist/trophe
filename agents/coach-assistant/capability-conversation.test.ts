...
describe('bounded capability selection',()=>{
 ...
 it.each([false,true])('keeps grounded records and selection with registry connected (exercise=%s)',async exercise=>{
  ...
  expect(result.ok,JSON.stringify(result)).toBe(true);expect(result.evidence.length).toBeGreaterThan(0);expect(result.telemetry.modelCalls).toBe(2);expect(result.telemetry.dataReads).toBeLessThanOrEqual(4);expect(personal).toHaveBeenCalledTimes(1);
 });
