...
export const photoFoodPortionSchema=z.object({loggedDate:z.string().date(),mealType:meal,mealSlot:z.enum(CANONICAL_MEAL_SLOTS).optional(),grams:z.number().finite().min(0.1).max(10000).refine(v=>Math.abs(Math.round(v*100)-v*100)<1e-7)}).strict().refine(value=>!value.mealSlot||mealSlotMatchesMealType(value.mealSlot,value.mealType),{message:'invalid meal slot'});
...
export const photoFoodReviewSchema=photoFoodPortionSchema.safeExtend({foodName:z.string().min(1).max(200),calories:z.number().finite().min(0).max(10000),proteinG:z.number().finite().min(0).max(1000),carbsG:z.number().finite().min(0).max(1000),fatG:z.number().finite().min(0).max(1000),fiberG:z.number().finite().min(0).max(1000),sugarG:z.number().finite().min(0).max(1000),confidence:z.number().finite().min(0).max(0.75),source:z.literal('photo_ai'),nutrition:z.literal('estimated'),portion:z.literal('explicit_user')}).strict();
...
export const photoFoodResultSchema=z.union([
 z.object({...common,storage:z.literal('database'),ok:z.literal(false),error:z.enum(['identity_clarification_required','invalid_input','forbidden','not_found','not_connected','version_conflict','expired','idempotency_conflict','uncertain','cancelled'])}).strict(),
 z.object({...common,ok:z.literal(true),proposal:photoFoodProposalSchema}).strict(),
 z.object({...common,ok:z.literal(true),snapshot:z.object({observationId:uuid,attachmentId:uuid,source:z.enum(['validated_photo_analysis','offline_fixture']),trust:z.literal('untrusted_image_data'),reviewRequired:z.literal(true),items:z.array(z.object({index:z.number().int().min(0).max(7),version:hash,foodName:z.string().min(1).max(200),estimatedGrams:z.number().finite().positive(),estimatedCalories:z.number().finite().min(0),estimatedProteinG:z.number().finite().min(0).optional(),confidence:z.number().finite().min(0).max(0.75),accuracyNote:z.string().max(500),identityStatus:z.enum(['identified','uncertain','unassessed']).default('unassessed')}).strict()).min(1).max(8)}).strict()}).strict(),
