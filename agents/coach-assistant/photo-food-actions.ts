...
export const photoFoodResultSchema=z.union([
 ...
 z.object({...common,ok:z.literal(true),snapshot:z.object({observationId:uuid,attachmentId:uuid,source:z.enum(['validated_photo_analysis','offline_fixture']),reviewRequired:z.literal(true),items:z.array(z.object({index:z.number().int().min(0).max(7),version:hash,foodName:z.string().min(1).max(200),estimatedGrams:z.number().finite().positive(),estimatedCalories:z.number().finite().min(0),confidence:z.number().finite().min(0).max(0.75),accuracyNote:z.string().max(500)}).strict()).min(1).max(8)}).strict()}).strict(),
