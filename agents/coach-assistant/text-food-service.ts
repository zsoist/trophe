    ...
    try {
      ...
      return await transaction(s, async tx => {
        ...
        for (let index = 0; index < items.length; index++) {
          ...
          for (const [actual, expected] of [[created.calories, items[index].calories], [created.proteinG, items[index].protein_g], [created.carbsG, items[index].carbs_g], [created.fatG, items[index].fat_g], [created.quantity, items[index].quantity], [created.parseConfidence, items[index].confidence]]) if (typeof actual !== 'number' || typeof expected !== 'number' || !Number.isFinite(actual) || Math.fround(actual) !== Math.fround(expected)) throw new Rejected('uncertain');
          for (const [key, actual] of [['fiber_g', created.fiberG], ['sugar_g', created.sugarG]] as const) {
            if (items[index].unavailable_nutrients?.includes(key)) {
              if (actual !== null) throw new Rejected('uncertain');
            } else if (typeof actual !== 'number' || !Number.isFinite(actual) || Math.fround(actual) !== Math.fround(items[index][key])) throw new Rejected('uncertain');
          }
        }
