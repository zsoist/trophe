...
it('AG4 one returned item cannot cover two requested portions',async()=>{
 h.provider.mockImplementation(async()=>wired({items:[candidateFor(40,'Oats')],needs_clarification:false,clarification_question:null}));
 const estimator=await estimatorFor();
 await expect(estimator([{name:'Double oats',foods:[{name:'Oats',grams:40},{name:'Oats',grams:40}]}],'en',new AbortController().signal)).rejects.toThrow('meal_advice_unavailable');
});
it('AG4 a quarter portion difference is not rounding',async()=>{
 h.provider.mockImplementation(async()=>wired({items:[candidateFor(120,'Chicken breast, cooked')],needs_clarification:false,clarification_question:null}));
 const estimator=await estimatorFor();
 await expect(estimator([{name:'Chicken',foods:[{name:'Chicken breast, cooked',grams:150}]}],'en',new AbortController().signal)).rejects.toThrow('meal_advice_unavailable');
});
