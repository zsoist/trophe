...
const providerSuccessSchema=z.object({
  responseModel:z.enum([LUNA_MODEL,TRANSCRIPTION_MODEL,HAIKU_MODEL,'gpt-live-1']),
...
export const pilotBudgetCommandSchema=z.discriminatedUnion('operation',[
  ...
  z.object({operation:z.literal('settle'),binding:pilotAttemptBindingSchema,usage:accountingUsageSchema,providerSuccess:providerSuccessSchema.optional()}).strict(),
]);
export type PilotBudgetCommand=z.infer<typeof pilotBudgetCommandSchema>;
export const pilotAttemptRecordSchema=z.object({
  binding:pilotAttemptBindingSchema,admissionDay:z.string().regex(/^\d{4}-\d{2}-\d{2}$/),state:z.enum(['reserved','dispatched','unknown','settled','released']),chargedNanoUsd:nano,usage:accountingUsageSchema.nullable(),accountingAlert:z.boolean(),unpricedModel:z.string().min(1).nullable().optional(),providerFailure:providerFailureDiagnosticSchema.optional(),
...
export function samePilotBinding(a:PilotAttemptBinding,b:PilotAttemptBinding):boolean {
  if(a.model==='gpt-live-1'&&b.model==='gpt-live-1'&&a.maxDurationSeconds!==b.maxDurationSeconds)return false;
...
export function decidePilotBudgetCommand(snapshot:{pilotId:string;budgetDay:string;capNanoUsd:number;chargedNanoUsd:number;turnAttemptCount:number;accountingBlocked:boolean;existing?:PilotAttemptRecord},raw:unknown):PilotBudgetDecision {
  ...
  const update=(state:PilotAttemptRecord['state'],chargedNanoUsd:number,usage:AccountingUsage|null,dispatchGranted=false,accountingAlert=false,providerFailure?:ProviderFailureDiagnostic):PilotBudgetDecision=>{
