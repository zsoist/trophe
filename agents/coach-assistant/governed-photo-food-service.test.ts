...
const binding: PilotAttemptBinding = { pilotId: id(1), actorId: id(2), attemptId: id(8), agentRunId: id(9), turnId: id(5), model: HAIKU_MODEL, pricingVersion: PHOTO_PILOT_PRICING_VERSION, reservedNanoUsd: PHOTO_ATTEMPT_RESERVATION_NANO_USD, requestHash: 'a'.repeat(64) };
...
describe('governed private photo composition', () => {
  it('analyzes one missing observation under the shared governor before publishing the settled read', async () => {
    ...
    const observations = { analyzeAndRecord: vi.fn(async (_scope, input) => { events.push('analyze'); expect(input.pilotBinding).toEqual(binding); return { result: { selectedPolicy: { provider: 'anthropic', model: HAIKU_MODEL, promptVersion: 'photo-analyze-v1' }, isFallback: false, usage: { inputTokens: 1, outputTokens: 1 }, rawStatus: 200, latencyMs: 1, output: {} }, observation: {} }; }) };
