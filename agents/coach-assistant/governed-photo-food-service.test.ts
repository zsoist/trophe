...
describe('governed private photo composition', () => {
  it('analyzes one missing observation under the shared governor before publishing the settled read', async () => {
    ...
    const observations = { analyzeAndRecord: vi.fn(async (_scope, input) => { events.push('analyze'); expect(input.pilotBinding).toEqual(binding); return { result: { selectedPolicy: { provider: 'openai', model: LUNA_MODEL, promptVersion: 'photo-analyze-v1' }, isFallback: false, usage: { inputTokens: 1, outputTokens: 1 }, rawStatus: 200, latencyMs: 1, output: {} }, observation: {} }; }) };
