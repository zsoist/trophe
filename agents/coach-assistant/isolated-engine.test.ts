...
describe('disposable CI authorized-records engine composition',()=>{
 ...
 it('executes actual handler/engine with authorized facts, explicit fixture metadata and no network',async()=>{
  ...
  const body=await (await handleCoachRequest(request(input),deps)).json();expect(body.ok).toBe(true);expect(body.dataSource).toBe('authorized_records');expect(body.evaluation).toMatchObject({transport:'injected_fixture',records:'authorized_records',semanticQualityVerified:false});expect(body.output.answer).toContain('500');expect(body.output.answer).not.toContain('Isolated transport fixture');expect(body.snapshot.subjectId).toBe(id(1));expect(body.proposals).toEqual([]);expect(body.receipts).toEqual([]);expect(fetch).not.toHaveBeenCalled();
