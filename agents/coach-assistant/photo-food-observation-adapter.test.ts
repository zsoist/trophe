...
const food={identity_status:'identified',name:'Rice',estimated_grams:100,estimated_calories:130,estimated_protein_g:2.7,estimated_carbs_g:28,estimated_fat_g:.3,estimated_fiber_g:.4,estimated_sugar_g:0,confidence:.7,source:'ai_estimate',accuracy_note:'Estimate; confirm grams.'};
...
describe('durable Photo Food observation adapter with injected SQL/runtime',()=>{
 ...
 it.each([undefined,'unassessed','confirmed'])('rejects new output with identity %s before issuing proof',async identity_status=>{
  const result=taskResult();(result.output.content[0] as {input:{foods:unknown[]}}).input.foods=[{...food,identity_status}];executeAiTask.mockResolvedValueOnce(result);
  await expect(runVerifiedPhotoFoodAnalysis(scope,{digest:imageDigest,bytes:imageBytes},{pilotBinding:binding(),invoke:vi.fn()})).rejects.toThrow('invalid_photo_output');
 });
 ...
 it('records normalized output only after current auth, attachment and durable generation evidence',async()=>{
  ...
  expect(f.statements.find(statement=>statement.includes('FROM private.coach_photo_food_observations o'))).toContain("g.prompt_version IN ('photo-analyze-v1','photo-analyze-v2','photo-analyze-v3')");
  expect(f.statements.find(statement=>statement.includes('FROM public.agent_runs g'))).toContain("g.prompt_version='photo-analyze-v3'");
 });
 ...
 it('loads exact private normalized bytes, reauthorizes and records through one composed call',async()=>{
  ...
  expect(completed.result).toMatchObject({generationId:id(5),selectedPolicy:{provider:'openai',model:LUNA_MODEL,promptVersion:'photo-analyze-v3'},isFallback:false,rawStatus:200});
 });
