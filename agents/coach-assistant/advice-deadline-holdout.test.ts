...
describe('nutrition advice estimator — existing Food pathway and shared authority', () => {
  it('returns completed options on the inner deadline and cancels the pending provider without starting another',async()=>{
    const normal=h.provider.getMockImplementation()!;
    let aborted=false;
    h.provider.mockImplementation(async(request)=>{
      if(request.prompt?.includes('Greek yogurt'))return new Promise((_,reject)=>{
        const stop=()=>{aborted=true;reject(request.signal.reason);};
        if(request.signal.aborted)stop();else request.signal.addEventListener('abort',stop,{once:true});
      });
      return normal(request);
    });
    const parent=new AbortController();const estimator=await estimatorFor();
    const meals=await estimator(THREE,'en',parent.signal,{timeoutMs:1000});
    expect(meals).toHaveLength(1);expect(meals[0].items.map(item=>item.grams)).toEqual([150,100]);
    expect(aborted).toBe(true);expect(parent.signal.aborted).toBe(false);expect(h.provider).toHaveBeenCalledTimes(2);
  });
