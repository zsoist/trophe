...
async function recipient(tx:Transaction,scope:Scope,missing:'not_found'|'forbidden'='forbidden'){
 ...
 if(!result.rows.length)throw new Rejected(missing);
 if(result.rows.length!==1)throw new Rejected('ambiguous_selection');
...
export function createCoachMessageService(database:Database,consumeRateLimit:(key:string,limit:number,seconds:number)=>Promise<{allowed:boolean;retryAfter:number}>):CoachMessageService{
 return {async execute(scope){
  ...
  try{return await database.transaction(async tx=>{
   ...
   const current=await recipient(tx,scope,operation.operation==='message.recipient'||operation.operation==='message.propose'?'not_found':'forbidden');
