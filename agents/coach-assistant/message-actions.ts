...
export const coachMessageResultSchema=z.union([
 z.object({ok:z.literal(false),error:z.enum(['invalid_input','forbidden','not_found','not_connected','version_conflict','expired','idempotency_conflict','rate_limited','cancelled','uncertain'])}).strict(),
...
export async function executeCoachMessageAction(actorId:string,raw:unknown,repository:CoachRepository,service:CoachMessageService,signal:AbortSignal):Promise<CoachMessageResult>{
 ...
 try{
  ...
  if(JSON.stringify(fresh)!==JSON.stringify(scope))return {ok:false,error:'forbidden'};
