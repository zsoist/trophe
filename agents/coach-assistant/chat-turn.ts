...
import { runVerifiedChatFinal, runVerifiedChatFinalWithIsolatedEngine } from './chat-final';
...
export async function runDurableChatTurn(request: CoachConversationRequest, options: RunOptions & { filterMemoryHistory?: (input: CoachConversationRequest) => CoachConversationRequest }, service: ReturnType<typeof createCoachChatService>, engine?: IsolatedCoachEngineBinding) {
  ...
  const generated = engine ? await runVerifiedChatFinalWithIsolatedEngine(request, options, scope, engine) : await runVerifiedChatFinal(request, options, scope);
