...
function fixture(){
 let revision='1';let allowed=true;let coachId=coach;let lost=false;let receiptFails=false;let recipientCalls=0;let authorizeThrough=Number.POSITIVE_INFINITY;let recipientRows=1;
 ...
 const tx={execute:async(q:Parameters<PgDialect['sqlToQuery']>[0])=>{const {sql,params:p}=new PgDialect().sqlToQuery(q);
  if(sql.includes('SELECT cp.coach_id')){recipientCalls++;return {rows:allowed&&recipientCalls<=authorizeThrough?Array.from({length:recipientRows},()=>({coach_id:coachId,full_name:'Test Coach',revision})):[]};}
 ...
 return {execute,rate,deleteMessage:(messageId:string)=>{delete messages[messageId];},state:()=>({messages,receipts}),lose:()=>{lost=true;},fail:()=>{receiptFails=true;},revoke:()=>{allowed=false;},revokeAfterNextAuthorization:()=>{authorizeThrough=recipientCalls+1;},ambiguous:()=>{recipientRows=2;},aba:()=>{revision='3';coachId=coach;}};
}
...
describe('human message SQL transaction double (no real sends)',()=>{
 ...
 it('asks for clarification when the assigned recipient is not unique',async()=>{
  const f=fixture();f.ambiguous();expect(await f.execute({...base,operation:'message.recipient'})).toMatchObject({ok:false,error:'ambiguous_selection'});expect(f.state()).toEqual({messages:{},receipts:{}});
 });
 it('reauthorizes immediately before storing and leaves no message or receipt after revocation',async()=>{
  const f=fixture();const apply=await proposal(f);f.revokeAfterNextAuthorization();expect(await f.execute(apply)).toMatchObject({ok:false,error:'forbidden'});expect(f.state()).toEqual({messages:{},receipts:{}});expect(f.rate).toHaveBeenCalledOnce();
 });
