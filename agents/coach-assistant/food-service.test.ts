...
const row={id:'00000000-0000-4000-8000-000000000001',userId:'00000000-0000-4000-8000-000000000002',foodId:null,foodName:'Fixture rice',quantity:1,qtyG:'250',calories:500,proteinG:10,carbsG:100,fatG:5,fiberG:2,sugarG:1,source:'manual',parseConfidence:null} as FoodLogRow;
function database(food:Record<string,number>[]=[],captureFails=false) {
  const statements:string[]=[];const updates:unknown[]=[];
  const db={select:()=>({from:()=>({where:()=>({limit:async()=>food})})}),update:()=>({set:(next:object)=>{updates.push(next);return {where:()=>({returning:async()=>[{...row,...next}]})};}}),insert:()=>({values:async()=>{if(captureFails)throw new Error('private correction payload');}}),execute:async(query:Parameters<PgDialect['sqlToQuery']>[0])=>{statements.push(new PgDialect().sqlToQuery(query).sql);return [];}} as unknown as FoodEditDatabase;
...
describe('extracted real Food calculation and writer with injected database operations',()=>{
  ...
  it('uses a savepoint for non-blocking correction capture inside the caller transaction',async()=>{
    const {db,statements,updates}=database([],true);const log=vi.spyOn(console,'error').mockImplementation(()=>{});
    try {
      const result=await applyFoodLogEdit({ctx:{db},existing:{...row,source:'photo_ai'},input:{grams:150},ownerUserId:row.userId!,correctedBy:row.userId!,transactional:true});
      expect(result.updated.qtyG).toBe('150');expect(result.captured).toBe(false);expect(updates).toHaveLength(1);
