...
For a completed, authorized photo observation, a typed `OpenConversationOutputError` rejection in the ordinary companion path returns only the deterministic review-available message and explicit explanation-unavailable limitation. No rejected prose, references or action intent escapes; no model retry or write occurs. Selected capabilities, provider/budget failures, cancellation and authorization failures retain their existing failure behavior. Final authorization still runs and revocation removes the recovered response. Live delivery remains subject to the normal attachment expiry and review → confirmation → canonical writer → receipt flow.

The follow-up real trial on 4cbf29a4 reached Photo review, explicit 220 g synthetic QA rice confirmation, receipt and canonical refetch successfully, but companion `numeric_prose` in `followUp` still showed a generic turn error. Recovery now applies to the typed output-rejection class, without weakening any validator: all model prose is discarded, diagnostics retained, and only authorized review availability is returned. Numeric follow-up and false execution-claim reproductions are covered offline alongside unbound references and final authorization revocation.
