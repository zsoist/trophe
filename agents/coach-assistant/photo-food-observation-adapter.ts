...
const PHOTO_PROMPT=readFileSync(join(process.cwd(),'agents/prompts/photo-analyze.v3.md'),'utf8').trim();
...
export async function runVerifiedPhotoFoodAnalysis(scope:PhotoFoodScope,image:{digest:string;bytes:Uint8Array},input:{requestId?:string;signal?:AbortSignal;pilotBinding:Readonly<PilotAttemptBinding>;invoke:(args:Parameters<ExecuteAiTaskInput<PhotoProviderOutput>['invoke']>[0]&{image:{bytes:Uint8Array;mediaType:'image/jpeg'|'image/png'|'image/webp'|'image/gif'}})=>ReturnType<ExecuteAiTaskInput<PhotoProviderOutput>['invoke']>}):Promise<{result:ExecuteAiTaskResult<PhotoProviderOutput>;proof:VerifiedPhotoFoodAnalysis}>{
 ...
 const result=await executeAiTask({task:'photo_analyze',prompt:PHOTO_PROMPT,context:{userId:scope.actorId,organizationId:scope.organizationId,requestId:input.requestId,metadata:{coachPhotoFood:{conversationId:scope.conversationId,attachmentId:scope.attachmentId,imageDigest,pilotId:binding.pilotId,pilotAttemptId:binding.attemptId,pilotAgentRunId:binding.agentRunId}}},invoke:args=>input.invoke({...args,signal:input.signal?AbortSignal.any([args.signal,input.signal]):args.signal,image:{bytes:bytes.slice(),mediaType:'image/jpeg'}})});
 if(result.selectedPolicy.provider!=='openai'||result.selectedPolicy.model!==LUNA_MODEL||result.selectedPolicy.promptVersion!=='photo-analyze-v3')throw new Error('invalid_photo_pipeline');
 ...
 if(foods.length!==uses[0].input!.foods!.length||foods.some(food=>food.needs_confirmation===true||food.identity_status==='unassessed'))throw new Error('invalid_photo_output');
...
export function createDatabasePhotoFoodObservationAdapter(database:Database,storage?:Pick<PrivateCoachImageStorage,'readNormalized'>):ObservationAdapter{
 ...
 const generation=await tx.execute(sql`SELECT g.generation_id FROM public.agent_runs g JOIN public.agent_runs budget ON budget.id=${analysis.pilotBinding.agentRunId}::uuid WHERE g.generation_id=${analysis.generationId}::uuid AND g.user_id=${scope.actorId}::uuid AND g.organization_id=${scope.organizationId}::uuid AND g.task_name='photo_analyze' AND g.status='completed' AND g.prompt_version='photo-analyze-v3' AND g.metadata#>>'{coachPhotoFood,conversationId}'=${scope.conversationId} AND g.metadata#>>'{coachPhotoFood,attachmentId}'=${scope.attachmentId} AND g.metadata#>>'{coachPhotoFood,imageDigest}'=${analysis.imageDigest} AND g.metadata#>>'{coachPhotoFood,pilotId}'=${analysis.pilotBinding.pilotId} AND g.metadata#>>'{coachPhotoFood,pilotAttemptId}'=${analysis.pilotBinding.attemptId} AND g.metadata#>>'{coachPhotoFood,pilotAgentRunId}'=${analysis.pilotBinding.agentRunId} AND budget.user_id=g.user_id AND budget.organization_id=g.organization_id AND budget.task_name='coach_pilot' AND budget.model=${LUNA_MODEL} AND budget.metadata#>>'{coachPilot,binding,pricingVersion}'=${PHOTO_PILOT_PRICING_VERSION} AND budget.metadata#>>'{coachPilot,binding,pilotId}'=${analysis.pilotBinding.pilotId} AND budget.metadata#>>'{coachPilot,binding,attemptId}'=${analysis.pilotBinding.attemptId} AND budget.metadata#>>'{coachPilot,state}'='dispatched' FOR SHARE OF g,budget`);
  ...
  async load(rawScope:PhotoFoodScope,signal:AbortSignal,tx:PhotoFoodObservationTransaction){
   ...
   // Luna is the only model admitted for new dispatch. The Haiku branch is a
   // deliberately narrow, read-only compatibility path for historical rows
   // whose binding carries the legacy pricing version; it can never create a
   // new observation or issue a provider call.
   const found=await tx.execute(sql`SELECT o.id,o.actor_id,o.subject_id,o.organization_id,o.conversation_id,o.attachment_id,o.revision,o.image_digest,o.generation_id,o.foods,o.active,a.normalized_digest AS attachment_digest,a.state AS attachment_state,false AS attachment_expired FROM private.coach_photo_food_observations o JOIN private.coach_attachment_uploads a ON a.id=o.attachment_id JOIN public.agent_runs g ON g.generation_id=o.generation_id JOIN public.agent_runs budget ON budget.id::text=g.metadata#>>'{coachPhotoFood,pilotAgentRunId}' WHERE o.actor_id=${scope.actorId}::uuid AND o.subject_id=${scope.subjectId}::uuid AND o.organization_id=${scope.organizationId}::uuid AND o.conversation_id=${scope.conversationId}::uuid AND o.attachment_id=${scope.attachmentId}::uuid AND o.active=true AND a.state='available' AND a.expires_at>clock_timestamp() AND g.user_id=o.actor_id AND g.organization_id=o.organization_id AND g.task_name='photo_analyze' AND g.status='completed' AND g.prompt_version IN ('photo-analyze-v1','photo-analyze-v2','photo-analyze-v3') AND g.metadata#>>'{coachPhotoFood,conversationId}'=o.conversation_id::text AND g.metadata#>>'{coachPhotoFood,attachmentId}'=o.attachment_id::text AND g.metadata#>>'{coachPhotoFood,imageDigest}'=o.image_digest AND g.metadata#>>'{coachPhotoFood,pilotId}'=${ASK_TROPHE_SHARED_PILOT_ID} AND budget.user_id=o.actor_id AND budget.organization_id=o.organization_id AND budget.task_name='coach_pilot' AND ((budget.model=${LUNA_MODEL} AND budget.metadata#>>'{coachPilot,binding,pricingVersion}'=${PHOTO_PILOT_PRICING_VERSION}) OR (budget.model=${HAIKU_MODEL} AND budget.metadata#>>'{coachPilot,binding,pricingVersion}'=${LEGACY_PHOTO_PILOT_PRICING_VERSION})) AND budget.metadata#>>'{coachPilot,binding,pilotId}'=${ASK_TROPHE_SHARED_PILOT_ID} AND budget.metadata#>>'{coachPilot,binding,attemptId}'=g.metadata#>>'{coachPhotoFood,pilotAttemptId}' AND budget.metadata#>>'{coachPilot,state}'='settled' FOR UPDATE OF o,a FOR SHARE OF g,budget`);
