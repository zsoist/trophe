...
export const workoutSetOperationSchema=z.discriminatedUnion('operation',[
  ...
  z.object({...base, setId:z.never().optional(),operation:z.literal('set.resolve'),sessionId:z.string().uuid().optional(),exerciseId:z.string().uuid().optional()}).strict(),
...
export const workoutSetValuesSchema=z.object({sessionId:z.string().uuid(),exerciseId:z.string().uuid(),exerciseName:z.string().trim().min(1).max(200),setNumber:z.number().int().positive(),reps:z.number().int().nullable(),weightKg:z.number().finite().nullable(),rpe:z.number().finite().nullable(),isWarmup:z.boolean().nullable(),isPr:z.boolean().nullable()}).strict();
export const workoutSetProposalSchema=z.object({id:z.string().uuid(),hash:z.string().regex(/^[a-f0-9]{64}$/),action:z.literal('workout.set.reps.update'),resource:z.object({kind:z.literal('workout_set'),id:z.string().uuid(),version}).strict(),before:workoutSetValuesSchema,after:workoutSetValuesSchema,expectedVersion:version,precondition:version,expiresAt:z.string().datetime({offset:true}),reviewRequired:z.literal(true)}).strict();
...
export async function executeWorkoutSetAction(actorId:string,raw:unknown,repository:CoachRepository,service:WorkoutSetService,signal:AbortSignal):Promise<WorkoutSetResult> {
  ...
  try {
    ...
    if(operation.operation==='set.resolve')return 'snapshot' in result&&(!operation.sessionId||result.snapshot.sessionId===operation.sessionId)&&(!operation.exerciseId||result.snapshot.exerciseId===operation.exerciseId)?result:fail('uncertain');
    ...
    if(operation.operation==='set.propose') {
      ...
      if(p.resource.id!==operation.setId||p.resource.version!==operation.resourceVersion||p.expectedVersion!==operation.resourceVersion||p.precondition!==operation.resourceVersion||p.after.reps!==operation.after.reps)return fail('uncertain');
