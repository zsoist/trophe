...
const reviewShape=object({loggedDate:date,mealType:meal,mealSlot:enumeration(CANONICAL_MEAL_SLOTS),grams:value=>finite(.1,10000)(value)&&Math.abs(Math.round((value as number)*100)-(value as number)*100)<1e-7,foodName:stringLength(1,200),calories:finite(0,10000),proteinG:finite(0,1000),carbsG:finite(0,1000),fatG:finite(0,1000),fiberG:finite(0,1000),sugarG:finite(0,1000),confidence:finite(0,.75),source:literal('photo_ai'),nutrition:literal('estimated'),portion:literal('explicit_user')},['mealSlot']);
const review:JsonCheck=value=>reviewShape(value)&&(!(value as {mealSlot?:string}).mealSlot||mealSlotMatchesMealType((value as import('./photo-food-contracts').PhotoFoodReview).mealSlot!,(value as import('./photo-food-contracts').PhotoFoodReview).mealType));
...
const item=object({index:integer(0,7),version:hash,foodName:stringLength(1,200),estimatedGrams:finite(Number.MIN_VALUE,Number.MAX_VALUE),estimatedCalories:finite(0,Number.MAX_VALUE),estimatedProteinG:finite(0,Number.MAX_VALUE),confidence:finite(0,.75),accuracyNote:stringLength(0,500),identityStatus:enumeration(['identified','uncertain','unassessed'])},['identityStatus','estimatedProteinG']);
...
const error=enumeration(['identity_clarification_required','invalid_input','forbidden','not_found','not_connected','version_conflict','expired','idempotency_conflict','uncertain','cancelled']);
