...
export interface IsolatedCoachEngineBinding {
 ...
 run(raw:unknown,options:RunOptions&{filterMemoryHistory?:(input:CoachConversationRequest)=>CoachConversationRequest;isolatedActionsEnabled?:boolean;workoutSetIntentsEnabled?:boolean}):Promise<CoachConversationResponse>;
}
...
export function createIsolatedCoachEngineBinding(env:Record<string,string|undefined>):IsolatedCoachEngineBinding{
 ...
 const binding:IsolatedCoachEngineBinding=Object.freeze({kind:'isolated_coach_engine_binding',async run(raw:unknown,options:RunOptions&{filterMemoryHistory?:(input:CoachConversationRequest)=>CoachConversationRequest;isolatedActionsEnabled?:boolean;workoutSetIntentsEnabled?:boolean}){
