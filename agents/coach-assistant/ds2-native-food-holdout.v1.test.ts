...
describe('native reference provenance', () => {
  it('distinguishes a branded official row from a generic model estimate', () => {
    // [working-copy remediation] Branded row + explicit MASS: the typed grams ARE the
    // portion, so the row's own macros stand — not an estimate. The original baseline
    // used a cup portion and asserted `estimate:false`; a non-mass unit has no proven
    // conversion, so that case is now an honest estimate (see the cup assertion below).
  ...
  it('keeps the original portion basis and grams instead of a 100 g masquerade', () => {
    // [working-copy remediation] A non-mass unit has NO proven conversion: local_db +
    // db_food_id proves the FOOD identity, not the cup→grams conversion, so the portion
    // is an honest model estimate (never validated_conversion) and the overall estimate
    // flag is truthful. Original baseline asserted `validated_conversion` from the row.
...
it('AG4 real Food catalogue miss does not certify a model-estimated cup conversion',async()=>{
 h.items=[{...chickenCandidate(),raw_text:'one cup cooked chicken breast',quantity:1,unit:'cup',estimated_grams:140,portion_explicit:true}];
 const f=makeGoverned();const result=await lookupNativeFoodReference({text:'one cup cooked chicken breast',language:'en'},scopeOf(f));
 expect(result.ok).toBe(true);if(!result.ok||result.outcome!=='reference')throw Error('expected reference');
 expect(result.references[0].estimate).toBe(true);expect(result.references[0].portionBasis).toBe('model_estimate');
});
it('AG4 inconsistent explicit mass is refused instead of labelled measured',()=>{
 expect(projectNativeFoodReference(item({quantity:0.5,unit:'kg',grams:87.5,calories:144.375,protein_g:27.125,carbs_g:0,fat_g:3.15}))).toBeNull();
});
it('AG4 rescale must remain inside the reference nutrient bounds',()=>{
 const ref=projectNativeFoodReference(item())!;
 expect(rescaleNativeFoodReference(ref,15000)).toBeNull();
});
