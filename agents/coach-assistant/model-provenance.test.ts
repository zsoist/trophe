...
describe('observed OpenAI model metadata through governed dispatcher',()=>{
 it.each([undefined,'gpt-5.6-luna-snapshot','different-model','',42])('preserves response model %s without request substitution',async(model)=>{
  const fetchImpl=vi.fn().mockResolvedValue(new Response(JSON.stringify({id:'resp_test',model,status:'completed',output:[{type:'function_call',name:'submit_result',arguments:'{"value":"ok"}'}],usage:{input_tokens:10,output_tokens:5}}),{status:200}));
