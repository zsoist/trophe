...
export function createPhotoFoodService(database:Database,observations?:PhotoFoodObservationPort,boundary?:IsolatedPhotoFoodBoundary):PhotoFoodService{return {async execute(scope){
 ...
 try{return await database.transaction(async tx=>{
  ...
  if(op.operation==='photo.food.read'||op.operation==='photo.food.propose'){
   ...
   const proposal:PhotoFoodProposal={id,hash:'',action,resource:{kind:'food_entry',id,version:op.resourceVersion},before:null,after:review,evidence:{observationId:o.id,observationRevision:o.revision,attachmentId:o.attachmentId,imageDigest:o.imageDigest,itemIndex:op.itemIndex,source:o.source},precondition:op.resourceVersion,expiresAt:new Date(clock.rows[0].expires).toISOString(),reviewRequired:true};proposal.hash=proposalHash(scope,proposal);
