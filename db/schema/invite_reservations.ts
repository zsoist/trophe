...
export const inviteReservations = pgTable('invite_reservations', {
  ...
  index('idx_invite_reservations_completed_recheck').on(t.completedAt).where(sql`status = 'completed' AND sealed_at IS NULL`), // 0046 (predicate must match migration exactly)
]).enableRLS();
