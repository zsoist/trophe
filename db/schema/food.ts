...
export const foodLog = pgTable('food_log', {
  ...
  pgPolicy('Clients manage own food log', { as: 'permissive', for: 'all', to: ['authenticated'],
...
export const foodDatabase = pgTable('food_database', {
  ...
  index('idx_food_db_name_trgm').using('gin', table.name.asc().nullsLast().op('gin_trgm_ops')),
  index('idx_food_db_name_el_trgm').using('gin', table.nameEl.asc().nullsLast().op('gin_trgm_ops')),
  index('idx_food_db_name_es_trgm').using('gin', table.nameEs.asc().nullsLast().op('gin_trgm_ops')),
  ...
  pgPolicy('Coaches insert food_database', { as: 'permissive', for: 'insert', to: ['authenticated'], withCheck: sql`private.is_staff()` }),
...
export const customFoods = pgTable('custom_foods', {
  ...
  pgPolicy('Users manage own custom foods', { as: 'permissive', for: 'all', to: ['authenticated'],
    using: sql`(created_by = auth.uid())`, withCheck: sql`(created_by = auth.uid())` }),
  pgPolicy('Clients see coach shared foods', { as: 'permissive', for: 'select', to: ['authenticated'], using: sql`(shared = true)` }),
]);
