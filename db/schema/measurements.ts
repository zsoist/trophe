...
export const measurements = pgTable('measurements', {
  ...
  pgPolicy('Clients manage own measurements', { as: 'permissive', for: 'all', to: ['authenticated'],
    using: sql`(user_id = auth.uid())`, withCheck: sql`(user_id = auth.uid())` }),
  pgPolicy('Coaches view client measurements', { as: 'permissive', for: 'select', to: ['authenticated'], using: sql`private.is_coach_of(user_id)` }),
]);
